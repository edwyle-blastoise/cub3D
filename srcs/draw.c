...
int	pixel_take(t_all *all, int x, int y, int side)
{
	...
	dst = NULL;
	dst = all->text[side].texture_addr + \
		(y * all->text[side].texture_width + x);
...
void	draw_wall(t_all *all, int x, int y, int side)
{
	...
	dy = all->text[side].texture_height * (all->params->wall_height - 2 * offset) \
		/ (2 * all->params->wall_height);
...
void	draw(t_all *all, int x, int side)
{
	...
	while (y < all->params->height)
	{
		...
		else if ((y >= (all->params->height - all->params->wall_height) / 2) \
			&& (y < (all->params->height + all->params->wall_height) / 2))
